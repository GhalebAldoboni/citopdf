...
## [1.7.0](https://github.com/GhalebAldoboni/citopdf/releases/tag/v1.7.0) · Home page, copy a page · 2026-09-20

- **Sharp pages at high zoom.** pdf.js rasters a page as one canvas and caps it at 48 MP, then lets CSS stretch that bitmap, so past about 380% the page went soft and never sharpened: at 1000% a Letter page was drawn at 6,237 px and shown across 16,320 device pixels, 0.76 pixels of render per pixel of screen. The part of the page you are looking at is now re-rendered at full device resolution into a second canvas laid over the page canvas (a full-resolution page canvas would be 344 MP and 1.4 GB). Sharp 255 ms after a scroll and 1.1 s after a zoom, the latter bounded by pdf.js's own render of the page beneath. Placement was verified by sweeping the crop against the page underneath: the difference is smallest at offset zero and grows either way, in both axes. It also restores the page background colour, which pdf.js drops whenever it caps a canvas — above 380% a document with tinted paper was being shown on white. Off below the cap, so ordinary reading is untouched: measured against a build without it, first page 409 ms vs 437, citation links 2.9 s vs 3.0, longest task in a 4 s scroll 52 ms vs 101. At 1000% it holds one crop of 23–29 MB, released when you zoom out, rotate, or leave the tab. Lowering the 48 MP cap to pdf.js's default 16 MP was tried and rejected: zooming to 400% then took 10.7 s instead of 230 ms.
- **Home page.** Clicking the toolbar icon opens Cito PDF's home instead of a bundled PDF, and focuses the home tab if one is already open. It has Open file, drag and drop anywhere, and a link box that takes a PDF link, a DOI or an arXiv id; recent documents with a first-page thumbnail, "page X of N" and when you last read them, reopening at the page you left; pin, remove, clear all and a filter; quick settings for day / night / AMOLED, the reader theme and embedded PDFs; tips; dark and light. It is also reachable from a Home button at the left of the viewer toolbar, from the icon's right-click menu, and it opens once on first install. Recents are recorded by the viewer in idle time (a 200 px JPEG thumbnail, about 22 KB, list capped at 50) and never leave the computer. Local files picked or dropped reopen through a stored file handle.
- **Copy page as image / Copy page text** in Chrome's own right-click menu, only inside the viewer. The image is the real page from pdf.js's bitmap (never the night-mode tint), re-rendered at 2× if the page is shown small; the text is rebuilt into lines in reading order with words hyphenated across lines rejoined, and a scanned page with no text layer says so. Verified on a 39-page paper: 1520 × 2252 px PNG, 3,932 characters for page 1. New permission: `clipboardWrite`.
- macOS Look Up (three-finger tap) opening the Dictionary app instead of the inline popover was investigated and traced to Chrome: when it works out which frame is under the pointer it scales the coordinates for a Retina display twice, so the lookup misses the viewer frame that sits under the PDF's own URL. Nothing in the text layer causes it. The findings are recorded in `bg/main/lookup.js`; selecting a word and choosing *Look Up* from the right-click menu shows the inline popover today.
...
- Night and AMOLED modes no longer invert photographs. Each raster image on a rendered page is located from pdf.js's operator list and its pixels sampled; images without a large white or black background (portraits and photos, unlike charts, diagrams and scans) get an un-inverted copy laid over them, so faces keep their colours while text and figures stay in night tones. Verified on a page with a portrait and a bar chart: only the portrait is restored.
- Keyed reference lists (`[AC91b] J. Schmidhuber. …`) without a References heading, as in web pages printed to PDF and many technical reports, now get citation popups. Scholar's analyzer only finds such a list under a heading and returns nothing otherwise (the original reader fails on them too). The viewer detects the keyed entries in the page text, builds the citation links and their targets itself in about two seconds, and lets Scholar's result replace them only if it finds more. Verified on the 97-page "Annotated history of modern AI" report: 709 references, 845 citation groups, where before there were none.
- Links inside PDFs (ORCID, DOI, arXiv…) open in a new tab. The viewer frame sits under the PDF's own URL and pdf.js was opening links inside that frame, so sites that forbid framing, such as orcid.org, showed "refused to connect".
- Papers whose reference list is followed by a supplement or appendix (PRL-style letters: references on page 7 of 19) now get citation popups. Scholar's analyzer expects the list near the end of the file and found nothing in such papers (the original Scholar reader fails on them too). The viewer now detects the supplement, analyses the paper up to its reference list first so links appear at once, then analyses the whole file and switches to that result only if it found the list, which also links the supplement's citations. Verified on arXiv 2503.12704: 52 references, links at 4 s, where before there were none.
- When Google Scholar rate-limits the network (it answers lookups with a 429 and its "unusual traffic" check), the popup now says so and links to the check, instead of the generic "error retrieving the results". One lookup is made per reference you open; nothing is prefetched.
- Citation links appear 2.5× sooner: the analyzer result is used the moment it arrives instead of after a 3 s quiet period, analysis starts within 0.5 s of the pages being in, and the per-paper scan runs while a ranged download finishes. 39-page ACM paper, 276 references: links at 3.1 s after open instead of 7.9 s.
