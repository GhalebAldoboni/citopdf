...
| A 1 GB scan still had to arrive whole: the relay streamed the full body into pdf.js and local files were read with one XMLHttpRequest. | The relay's first request is a 1 MB `Range`; a 206 answer gives the size and pdf.js reads by range (`bg/main/embed.js`, `embedded.js`): pages in view first, then the remaining chunks one at a time in the background until the file is complete (pdf.js auto-fetch, which needs `disableStream` since there is no full stream). Chunks are 1 MB above 48 MB, 256 KB above 8 MB. Local files: Chrome slices `file://` reads by range, so the size is found with about 40 one-byte probes and the file is read the same way (under 4 MB it is read whole). Citation analysis runs on files up to 1 GB. | 200 MB PDF: first page at 0.35 s over HTTP and 0.28 s from disk; page 3 renders on demand; the whole file is local after about 6 s (201 range requests). |
| A tab left in the background kept every rendered page, its thumbnails and pdf.js's font and image caches for as long as it was open. | After five minutes hidden, all rendered pages except the current one are released, thumbnails and worker caches cleared, and rendering held until the tab is visible again (`bg/main/perf.js`). The file bytes stay in memory so reading resumes without any network. | 39-page paper: 7 rendered pages → 1, JS heap 90 MB → 26 MB while hidden; pages in view re-render on return. |
| pdf.js parsed every document on the main thread: a theme script deleted `URL.prototype.origin`, so pdf.js's same-origin check for its worker failed and it silently fell back to the in-page "fake worker". Parsing and rendering competed with scrolling and the citation overlay. | The origin property is left intact and the viewer's file-origin check allows the extension origin, so pdf.js starts its real Web Worker (`bg/main/replace.js`, `viewer.js`). | First page of a local 7 MB paper: 540 ms → 410 ms; parsing no longer blocks the UI thread. |
