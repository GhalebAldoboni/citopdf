...
|---|---|---|
| The original viewer used a blocking `webRequest` listener, which Manifest V3 forbids for sideloaded extensions, and read misspelt event fields, so PDFs never auto-opened. | Interception uses `declarativeNetRequest` rules matched on response headers: PDF content types, generic binary types with a `.pdf` path, and attachments whose file name ends in `.pdf`. | Web PDFs open in the viewer with no runtime listener at all. |
